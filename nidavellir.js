  ...
  return declare(
    ...
    ],
    {
      ...
      clearPossible() {
        ...
        if (this._distinctionExplorerCards != null) this._distinctionExplorerModal.destroy();
        if (this._discardDialog != null) this._discardDialog.hide();
      ...
      setupInfoPanel() {
        ...
        $('show-overview').addEventListener('click', () => this.showOverview());
        this.addTooltip('show-overview', '', _('Show score details.'));
      ...
      setupHelper() {
        let contents = this.format_block('jstpl_helpModal', {
          ...
          warriorsText: _(
            'Their Bravery Value is equal to the sum of their Bravery Points, to which the Elvaland who gets majority in ranks in the Warrior column, add his coin of highest value.',
          ),
          warriorsText2: _(
            'In case of a tie, all tied Elvalands add their highest value coin to their Warrior Bravery Value.',
          ),
          hunterText: _('Their Bravery Value is equal to the number of Hunters squared.'),
          minersText: _(
            'Their Bravery Value is equal to the sum of their Bravery Points multiplied by the number of ranks in their column.',
          ),
          ...
          if (log && args && !args.processed) {
            ...
            if (args.card_class !== undefined) {
              ...
              if (args.card_class_symbol[0] == 6) {
                ...
                args.card_class_symbol = dojo.string.substitute(
                  "<span class='card-class-symbol class-${class}'></span>",
                  { class: args.card_class_symbol },
                );
              }
            }
            ...
            if (args.card2_class !== undefined) {
              ...
              args.card2_class_symbol = dojo.string.substitute(
                "<span class='card-class-symbol class-${class}'></span>",
                { class: args.card2_class_symbol },
              );
            }
            ...
            // Coin icons
            var coinKeys = Object.keys(args).filter(
              (key) => key.substr(0, 4) == 'coin' && key.substr(-4) != 'type' && args[key + '_type'] !== undefined,
            );
