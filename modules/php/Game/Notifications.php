...
class Notifications
{
  ...
  public static function newOrder($order)
  {
    $msg = [
      ...
      '',
      clienttranslate(
        'Recruiting of current tavern will take place in following order : ${player_name1} then ${player_name2}'
      ),
      clienttranslate(
        'Recruiting of current tavern will take place in following order : ${player_name1}, ${player_name2} then ${player_name3}'
      ),
      clienttranslate(
        'Recruiting of current tavern will take place in following order : ${player_name1}, ${player_name2}, ${player_name3} then ${player_name4}'
      ),
      clienttranslate(
        'Recruiting of current tavern will take place in following order : ${player_name1}, ${player_name2}, ${player_name3}, ${player_name4} then ${player_name5}'
      ),
    ];
    $data = [
      'order' => $order,
  ...
  public static function tradeGems($bid, $bidders, $trades)
  {
    $msgs = [
      ...
      '',
      clienttranslate(
        '${player_name} and ${player_name2} both bade with their ${bid} coin, and trade their gems as a tie'
      ),
      clienttranslate(
        '${player_name}, ${player_name2} and ${player_name3} all bade with their ${bid} coin, and trade their gems as a tie (${player_name2} keeps its gem)'
      ),
      clienttranslate(
        '${player_name}, ${player_name2}, ${player_name3} and ${player_name4} all bade with their ${bid} coin, and trade their gems as a tie'
      ),
      clienttranslate(
        '${player_name}, ${player_name2}, ${player_name3}, ${player_name4} and ${player_name5} all bade with their ${bid} coin, and trade their gems as a tie (${player_name3} keeps its gem)'
      ),
    ];
  ...
  public static function tradeCoin($player, $coinMin, $coinMax, $newCoin)
  {
    ...
    self::notifyAll(
      'tradeCoin',
      clienttranslate(
        '${player_name} uses ${coin_min}${coin_min_type} to trade ${coin_max}${coin_max_type} into ${coin_new}${coin_new_type}'
      ),
      [
        'player' => $player,
  ...
  public static function transformCoin($player, $coinMax, $newCoin)
  {
    self::notifyAll(
      'tradeCoin',
      clienttranslate('${player_name} transforms ${coin_max}${coin_max_type} into ${coin_new}${coin_new_type}'),
      [
        'player' => $player,
        'coin_max' => $coinMax['value'],
        'coin_max_type' => $coinMax['type'],
  ...
  public static function discardCards($player, $cards)
  {
    if ($cards instanceof \NID\Cards\AbstractCard) {
      ...
      self::notifyAll(
        'discardCards',
        clienttranslate(
          '${player_name} discards ${card_class}${card_class_symbol} and ${card2_class}${card2_class_symbol}'
        ),
        [
          'player' => $player,
          'card' => $t[0],
  ...
  public static function discardHofud($player, $card, $warriors)
  {
    if (is_null($card)) {
      self::notify($player->getId(), 'discardHofud', '', [
        'player' => $player,
        'warriors' => array_map(function ($card) {
          return $card->getId();
        }, $warriors),
      ]);
    } else {
      self::notifyAll(
        'discardHofud',
        clienttranslate('${player_name} discards ${card_class}${card_class_symbol} for Hofud\'s effect'),
        [
          'player' => $player,
          'card' => $card,
          'warriors' => array_map(function ($card) {
  ...
  public static function distinctionTie($distinction, $maxRank)
  {
    self::notifyAll(
      'distinctionTie',
      clienttranslate(
        '${n} players are tied to the majority of ${card_class}${card_class_symbol} hence no one get the distinction card'
      ),
      [
        'n' => count($maxRank),
        'card' => $distinction,
  ...
  public static function distinction($player, $card)
  {
    self::notifyAll(
      'distinction',
      clienttranslate(
        '${player_name} has the strict majority of ${card_class}${card_class_symbol} and wins the corresponding distinction'
      ),
      [
        'player' => $player,
        'card' => $card,
  ...
  public static function distinctionCoin($player, $oldCoin, $newCoin)
  {
    self::notifyAll(
      'tradeCoin',
      clienttranslate(
        '${player_name} trades its ${coin_max}${coin_max_type} with the special ${coin_new}${coin_new_type}'
      ),
      [
        'player' => $player,
  ...
  public static function discardTradingCoin($player, $tradingCoin)
  {
    self::notifyAll(
      'discardCoin',
      clienttranslate('${player_name} discard its ${coin_max}${coin_max_type} (Jarnglofi\'s effect)'),
      [
        'player' => $player,
  ...
  public static function discardCardExplorerDistinction($card)
  {
    self::notifyAll(
      'discardCards',
      clienttranslate('A ${card_class}${card_class_symbol} is discarded from the age 2 deck'),
      [
        'card' => $card,
      ]
    );
  ...
  public static function newEnlistOrder($order)
  {
    $msg = [
      ...
      '',
      clienttranslate(
        'Enlisting of mercenaries will take place in following order : ${player_name1} then ${player_name2}'
      ),
      clienttranslate(
        'Enlisting of mercenaries will take place in following order : ${player_name1}, ${player_name2} then ${player_name3}'
      ),
      clienttranslate(
        'Enlisting of mercenaries will take place in following order : ${player_name1}, ${player_name2}, ${player_name3} then ${player_name4}'
      ),
      clienttranslate(
        'Enlisting of mercenaries will take place in following order : ${player_name1}, ${player_name2}, ${player_name3}, ${player_name4} then ${player_name5}'
      ),
    ];
    $data = [
      'order' => $order,
  ...
  public static function denyCapture($player, $card, $giant)
  {
    self::notifyAll(
      'denyCapture',
      clienttranslate('${player_name} does not capture the dwarf and lose the giant\'s effect'),
      [
        'player' => $player,
        'card' => $card,
        'giant' => $giant,
  ...
  public static function capture($player, $giant, $card)
  {
    self::notifyAll(
      'capture',
      clienttranslate('${player_name} captures ${card_class}${card_class_symbol} and trigger ${giant_name}\'s effect'),
      [
        'player' => $player,
        'card' => $card,
  ...
  public static function wasteLoki($player, $cardId)
  {
    self::notifyAll(
      'wasteLoki',
      clienttranslate('${player_name} discards Loki token since he recruited another card'),
      [
        'player' => $player,
        'cardId' => $cardId,
      ]
  ...
  public static function exchangeCard($player, $card1, $card2)
  {
    self::notifyAll(
      'exchangeCard',
      clienttranslate(
        '${player_name} exchanges ${card_class}${card_class_symbol} and ${card2_class}${card2_class_symbol}'
      ),
      [
        'player' => $player,
        'card' => $card1,
  ...
  public static function putBackCard($player, $card)
  {
    self::notifyAll(
      'putBackCard',
      clienttranslate('${player_name} put back ${card_class}${card_class_symbol} at the bottom of the deck'),
      [
        'player' => $player,
        'card' => $card,
      ]
